...
-- Keep users table but add missing columns
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'users' AND column_name = 'self_verified') THEN
        ALTER TABLE users ADD COLUMN self_verified BOOLEAN DEFAULT false;
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                   WHERE table_name = 'users' AND column_name = 'voting_eligible') THEN
        ALTER TABLE users ADD COLUMN voting_eligible BOOLEAN DEFAULT false;
    END IF;
END $$;
